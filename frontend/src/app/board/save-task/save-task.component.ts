...
export class SaveTaskComponent implements OnInit {
  ...
  constructor(private board: BoardService, private router: Router, private activateRoute: ActivatedRoute) {
  ...
  addTaskImg() {
    if (!this.taskData.name || !this.taskData.description) {
      ...
      this.board.addTaskImg(data).subscribe(
        (res) => {
          this.router.navigate(['/listTask', this.id]);
